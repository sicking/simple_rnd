...
macro_rules! impl_rng_float_between {
  ($fty: ty, $uty: ty, $mantissa_bits: expr, $exp_bits: expr, $gen_func: ident, $float_between_func: ident) => (
    pub fn $float_between_func<R: Rng + ?Sized>(rng: &mut R, start: $fty, end: $fty) -> $fty {
...
impl_rng_float_between!(f64, u64, 52, 11, gen_u64, rng_float64_between);
impl_rng_float_between!(f32, u32, 23, 8, gen_u32, rng_float32_between);
